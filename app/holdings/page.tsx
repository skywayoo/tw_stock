...
import { Plus, ChevronDown, ChevronUp, Pencil, TrendingUp, Percent, ArrowDownUp, GripVertical } from 'lucide-react';
import {
  DndContext,
  closestCenter,
  PointerSensor,
...
const ORDER_KEY = 'holdings_order';
...
interface HoldingCardProps {
  ...
  isOpen: boolean;
  onToggle: () => void;
  onToggleDeduct: (id: string, cur: boolean) => void;
}

function SortableHoldingCard(props: HoldingCardProps) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: props.h.id });
  const style = {
  ...
  const { h, p, exDivs, activeLendings, sblInfo, isOpen, onToggle, onToggleDeduct } = props;
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`rounded-2xl overflow-hidden shadow-lg ${limitUp ? 'ring-2 ring-red-500 bg-red-950' : limitDown ? 'ring-2 ring-green-600 bg-green-950' : 'bg-gray-900'}`}
    >
      {/* Main Card */}
      <div className="p-4">
        {/* Top row: drag handle + name + edit */}
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-start gap-2">
            ...
            <button
              {...attributes}
              {...listeners}
              className="mt-1 touch-none text-gray-600 active:text-gray-400 cursor-grab active:cursor-grabbing"
              aria-label="拖曳排序"
            >
...
export default function HoldingsPage() {
  ...
  // Load saved order once holdings arrive
  useEffect(() => {
    if (holdings.length === 0) return;
    const saved = loadOrder();
    const holdingIds = holdings.map((h) => h.id);
    // Merge: saved order first, then any new holdings not in saved
    const merged = [
      ...saved.filter((id) => holdingIds.includes(id)),
      ...holdingIds.filter((id) => !saved.includes(id)),
    ];
    setOrderedIds(merged);
  }, [holdings]);

  const orderedHoldings = orderedIds
              ...
              {orderedHoldings.map((h) => {
                ...
                return (
                  <SortableHoldingCard
                    ...
                    sblInfo={sblInfo}
                    isOpen={expanded === h.id}
                    onToggle={() => setExpanded(expanded === h.id ? null : h.id)}
                    onToggleDeduct={toggleDeduct}
                  />
                );
              })}
            </SortableContext>
